...
} from "../assets/js/analysis/image-analysis.js?v=20260916-ocr-regression-01";
import {
  createOcrWorker,
...
} from "../assets/js/ocr/ocr.js?v=20260916-ocr-regression-01";
...
async function drawImage(filename) {
  ...
  const size = getAnalysisCanvasSize(image.naturalWidth, image.naturalHeight);
...
async function run() {
  ...
  const manifest = await fetch(`${root}/expected.json?v=20260916-01`).then(r => r.json());
  ...
  try {
    ...
    for (let index = 0; index < entries.length; index++) {
      ...
      const { canvas, context } = await drawImage(filename);
  ...
  const summary = {
    build: "20260916-ocr-regression-01",
